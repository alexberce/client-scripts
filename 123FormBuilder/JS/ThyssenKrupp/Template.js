(function(){ 
  ...
  jQuery(document).ready(function(){
    setTimeout(() => {
      ...
      jQuery('[data-id="51234980"] table').eq(0).attr('cellspacing', '0px');
      ...
      //Hours Worked Event
      jQuery('[data-id="51234986"] [data-role="i123-input"]').on('input', () => {
        calculateAndUpdateHours();
      });

      // Expenses
      jQuery('[data-id="51234987"] [data-role="i123-input"]').on('input', () => {
        calculateAndUpdateExpenses();
      });

      // Expenses
      jQuery('[data-id="51234990"] [data-role="i123-input"]').on('input', () => {
        calculateAndUpdateNightRatePayment();
      });
    }, 10);
  });
  ...
  function calculateAndUpdateHours(){
    ...
    jQuery('[data-id="51234991"] td:nth-child(2) span').text(String(zeroPad(totalHours, 2)) + ':' + String(zeroPad(totalMinutes, 2)));
  }

  function calculateAndUpdateExpenses(){
    let valueArray = priceCalculator(50640467);

    jQuery('[data-id="51234991"] td:nth-child(3) span').text(String(zeroPad(valueArray[0], 3)) + '.' + String(zeroPad(valueArray[1], 2, 'after')));
  }

  function calculateAndUpdateNightRatePayment(){
    let valueArray = priceCalculator(50640562);

    jQuery('[data-id="51234991"] td:nth-child(6) span').text(String(zeroPad(valueArray[0], 3)) + '.' + String(zeroPad(valueArray[1], 2, 'after')));
  }
